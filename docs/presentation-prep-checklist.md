# Pre-Presentation Prep — the intended operator Demo (this week)
...
- [ ] Screens the intended operator will actually see in the demo, in priority order: login → home (three-station front door) → live board → labels. Polish those four; skip the rest.

## 2. Demo readiness

- [ ] Seed real-looking demo data (the intended operator's client, a plausible crew roster with photos, a production dated this week). No "Test Person 1".
...
- [ ] Hand him `operator-update-2026-07.md` (or a PDF of it) as the leave-behind; drive the meeting from the live app, not the doc.
